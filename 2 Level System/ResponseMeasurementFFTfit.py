...
nonhermfactor = 1 / ( 0.03 * 7.58 * (1/2+0.125) )#
...
sz_real_data = np.array(y0)

sz_real_data = np.append(sz_real_data, np.array(y3))
...
print("FIGURE RATIO REAL DATA: ", 1.122 * (popt1[1] * hermfactor) / (popt[1] * nonhermfactor))
...
#print("herm_fitted_amplitude: ", herm_fitted_amplitude)
params1 = Parameters()
...
print("herm_fitted_amplitude: ", herm_fitted_amplitude)
print("nonherm_fitted_amplitude: ", nonherm_fitted_amplitude)
...
t_plot = np.linspace(0, t_max / 2 / np.pi, num=1000)
...
ax1.fill_between(np.fft.fftfreq(len(ynhf0), d=x0[1])[int(len(ynhf0) / 2) + 1:int(len(ynhf0))],
                 ((1 - 2 / (np.exp(
                     prefactor * np.fft.fftfreq(len(ynhf0), d=x0[1])[int(len(ynhf0) / 2) + 1:int(len(ynhf0))] / (Temp -120 *10**(-6))) + 1))
                   * np.real(np.fft.fft(ynhf0, norm="backward"))[int(len(ynhf0) / 2) + 1:int(len(ynhf0))]),
                 ((1 - 2 / (np.exp(
                     prefactor * np.fft.fftfreq(len(ynhf0), d=x0[1])[int(len(ynhf0) / 2) + 1:int(len(ynhf0))] / (Temp +120 *10**(-6))) + 1))
                   * np.real(np.fft.fft(ynhf0, norm="backward"))[int(len(ynhf0) / 2) + 1:int(len(ynhf0))]),
