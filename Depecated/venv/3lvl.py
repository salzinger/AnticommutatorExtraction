...
def sigmaz(j, N):
    oplist = np.full(N, identity(3))
    oplist[j] = Qobj([[1, 0, 0], [0, -1, 0]])
    return tensor(oplist)


def sigmax(j, N):
    oplist = np.full(N, identity(3))
    oplist[j] = Qobj([[0, 1, 0], [1, 0, 0]])
    return tensor(oplist)


def sigmay(j, N):
    oplist = np.full(N, identity(3))
    oplist[j] = Qobj([[0, -1j, 0], [1j, 0, 0]])
    return tensor(oplist)
...
def H0(omega, N):
    H = 0
    for j in range(0, N):
        H += 1 * omega / 2 * sigmaz(j, N) + omega/2 * [[0, 0, 0], [0, 0, 1]]
    return H
...
random_phase = 0.01 * np.random.randn(t.shape[0])
